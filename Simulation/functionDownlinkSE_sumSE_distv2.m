function SE = functionDownlinkSE_sumSE_distv2(bk,Ck,preLogFactor,L,K,N,D,rhomax,tau_p)
...
while (diff>0.1) || (diff<0)
    ...
    cvx_begin quiet
    variable rho3(L,K,N)
    variable rho2(sum(La),N) %non-zero portions of the main optimization variable rho3
    variable sss(K,1)
    minimize sum(sss)
    subject to
    for n = 1:N
