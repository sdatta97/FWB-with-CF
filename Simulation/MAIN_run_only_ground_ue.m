...
BETA =0;      %RANDOM GAINS,  practical Rician, %large-scale-fadings = 1,  if 0. practical channel
K_Factor = 9;         %dB -- %rician factor Ground UE  % if beta_gains=1
RAYLEIGH=0;   %1= rayleigh, % 0=rician
Perf_CSI =1;
cov_area = 1; %0.25; % 4; %km
%%
TAU_P_K_by_two = 0; %1;  
CH_estimation = 0;  % 1= have channel estimation
%%
LB=1;  %Lower bound
UB =1;  %Upper bound
...
%%  define
NMSE = zeros(LOOP,1);
SE_UB_each = zeros(K,LOOP,length(ASD_VALUE),length(Kt_Kr_vsUE ),nbr_setups);  %UPPER-bound
SE_LB_each = zeros(K,LOOP,length(ASD_VALUE),length(Kt_Kr_vsUE ),nbr_setups);  %lower-bound
SE_LB = zeros(LOOP,length(ASD_VALUE),length(Kt_Kr_vsUE ),nbr_setups);  %lower-bound
SE_monte_impCSI = zeros(LOOP,length(ASD_VALUE),length(Kt_Kr_vsUE ),nbr_setups);  %upper bound

SNR_NUM_LB7 = zeros(K,LOOP,length(ASD_VALUE),length(Kt_Kr_vsUE ),nbr_setups);  %lower-bou
SNR_DEN_LB7= zeros(K,LOOP,length(ASD_VALUE),length(Kt_Kr_vsUE ),nbr_setups);
HI_UE_rx7= zeros(K,LOOP,length(ASD_VALUE),length(Kt_Kr_vsUE ),nbr_setups);
HI_AP_tx7= zeros(K,LOOP,length(ASD_VALUE),length(Kt_Kr_vsUE ),nbr_setups);
BU7= zeros(K,LOOP,length(ASD_VALUE),length(Kt_Kr_vsUE ),nbr_setups);
INTERFERENCE_UAV_GUE_EACH7= zeros(K,K,LOOP,length(ASD_VALUE),length(Kt_Kr_vsUE ),nbr_setups);

%% all setups
for n = 1:nbr_setups
    disp(['setup:'   num2str(n)]);
    %% AVERAGING LARGE-SCALE-FADING
    %R = zeros(N_AP,N_AP,AP,UE);    % h_LOS = zeros(N,AP,UE);
    [channelGain_GUE,R_GUE,h_LOS_GUE,K_Rician,PLOS_GUE] = channel_cellfree_GUE3(K,L,N,ASD_VALUE,ASD_CORR,RAYLEIGH,BETA,K_Factor,cov_area,Band);
    R10 = R_GUE;
    h_LOSall = h_LOS_GUE;
    %% PLOS cal
    %         % GUE
    ...
    for iter = 1:LOOP    
        for iHI = 1:length(Kt_Kr_vsUE)          
            ...
            for iASD = 1: length(ASD_VALUE)
                ...
                disp(['SETUP: ',num2str(n), ' SNR: '   num2str(snr_db(iter))]);
                snr = snr_linear;
                % power allocation
                eta = zeros(L,K);
                ...
                if LB==1
                    ...
                    [SE_LB_ALL, SNR_NUM_LB7(1:K,iter,iASD,iHI,n), SNR_DEN_LB7(1:K,iter,iASD,iHI,n), HI_UE_rx7(1:K,iter,iASD,iHI,n), HI_AP_tx7(1:K,iter,iASD,iHI,n), BU7(1:K,iter,iASD,iHI,n),INTERFERENCE_UAV_GUE_EACH7(1:K,1:K,iter,iASD,iHI,n)] = function_LB_impCSI(K,L,N,eta,h_LOS,R,psi_HI,eta_p,PHI,k_t2,k_r2_UE,gamma, gamma_MAT, beta_actual, beta_actual_MAT, C_ERR, GAMMA_NLOS );
                    
                    SE_LB_each(1:K,iter,iASD,iHI,n) = tau_factor*SE_LB_ALL;
                    SE_LB(iter,iASD,iHI,n) = tau_factor*sum(SE_LB_ALL);
...
figure
hold on;
plot(snr_db,Band*sum_SE_UB(:,1)/K,'-b*');
plot(snr_db,Band*sum_SE_LB(:,1)/K,'-r*');
xlabel('SNR');
ylabel('per user rate');
legend('Ergodic','Closed-form');
title(['L= ', num2str(L),'N= ', num2str(N),' K=', num2str(K)]);
