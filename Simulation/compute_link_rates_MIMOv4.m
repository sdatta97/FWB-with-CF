function [rate_dl, eta_eq] = compute_link_rates_MIMOv4(params,channel_dl, channel_est_dl,channel_dl_mmW, channel_est_dl_mmW,sub6ConnectionState)
M = size(channel_dl,1);
...
Kt = params.Kt;
Kr_mmW = params.Kr_mmW;
Kr_sub6 = params.Kr_sub6;
...
for n_idx = 1:num_sc_sub6
    ...
    %             DS_mmW(k,n) = p_d*norm(reshape(D_mmW_mmW(k,k,n,:),[1,N_UE_mmW]))^2;
                DS_mmW(k,n) = p_d*Kr_mmW*Kt*(abs(D_mmW_mmW(k,k,n,n,n_idx)))^2;
                HI_mmW(k,n) = p_d*(1-Kr_mmW*Kt)*(abs(D_mmW_mmW(k,k,n,n,n_idx)))^2;
                for nn = 1:N_UE_mmW
    %                 if (nn~=n)
    ...
    %                     MSI_mmW(k,n) = MSI_mmW(k,n) + p_d*norm(reshape(D_mmW_mmW(k,k,nn,:),[1,N_UE_mmW]))^2;
                        MSI_mmW(k,n) = MSI_mmW(k,n) + p_d*Kr_mmW*Kt*(abs(D_mmW_mmW(k,k,n,nn,n_idx)))^2;
                        HI_mmW(k,n) = HI_mmW(k,n) + p_d*(1-Kr_mmW*Kt)*(abs(D_mmW_mmW(k,k,n,nn,n_idx)))^2;
                    end
                end
                for q = 1:K_mmW
                    if (q~=k && sub6ConnectionState(q)==1)
                      MUI_mmW(k,n) = MUI_mmW(k,n) + p_d*Kr_mmW*Kt*norm(reshape(D_mmW_mmW(k,q,n,:,n_idx),[1,N_UE_mmW]))^2;
                      HI_mmW(k,n) = HI_mmW(k,n) + p_d*(1-Kr_mmW*Kt)*norm(reshape(D_mmW_mmW(k,q,n,:,n_idx),[1,N_UE_mmW]))^2;
                    end
                end
                for q = 1:K-K_mmW
                   MUI_mmW(k,n) = MUI_mmW(k,n) + p_d*Kr_mmW*Kt*norm(reshape(D_mmW_sub6(k,q,n,:,n_idx),[1,N_UE_sub6]))^2;
                   HI_mmW(k,n) = HI_mmW(k,n) + p_d*(1-Kr_mmW*Kt)*norm(reshape(D_mmW_sub6(k,q,n,:,n_idx),[1,N_UE_sub6]))^2;
                end
                snr_num_mmW(k,n) = DS_mmW(k,n);
                snr_den_mmW(k,n) = MSI_mmW(k,n) + MUI_mmW(k,n) + HI_mmW(k,n) + noise_mmW(k,n);
    ...
    for k = 1:K-K_mmW
        ...
        %         DS_sub6(k,n) = p_d*norm(reshape(D_sub6_sub6(k,k,n,:),[1,N_UE_sub6]))^2;
                DS_sub6(k,n) = p_d*Kr_sub6*Kt*(abs(D_sub6_sub6(k,k,n,n,n_idx)))^2;
                HI_sub6(k,n) = p_d*(1-Kr_sub6*Kt)*(abs(D_sub6_sub6(k,k,n,n,n_idx)))^2;
                for nn = 1:N_UE_sub6
        %             if (nn~=n)
        %             if (nn>n)
        ...
        %                 MSI_sub6(k,n) = MSI_sub6(k,n) + p_d*norm(reshape(D_sub6_sub6(k,k,nn,:),[1,N_UE_sub6]))^2;
                        MSI_sub6(k,n) = MSI_sub6(k,n) + p_d*Kr_sub6*Kt*(abs(D_sub6_sub6(k,k,n,nn,n_idx)))^2;
                        HI_sub6(k,n) = HI_sub6(k,n) + p_d*(1-Kr_sub6*Kt)*(abs(D_sub6_sub6(k,k,n,nn,n_idx)))^2;
                    end
                end
                for q = 1:K_mmW
                    if (q~=k && sub6ConnectionState(q)==1)
                      MUI_sub6(k,n) = MUI_sub6(k,n) + p_d*Kr_sub6*Kt*norm(reshape(D_sub6_mmW(k,q,n,:,n_idx),[1,N_UE_mmW]))^2;
                      HI_sub6(k,n) = HI_sub6(k,n) + p_d*(1-Kr_sub6*Kt)*norm(reshape(D_sub6_mmW(k,q,n,:,n_idx),[1,N_UE_mmW]))^2;
                    end
                end
                for q = 1:K-K_mmW
                   MUI_sub6(k,n) = MUI_sub6(k,n) + p_d*Kr_sub6*Kt*norm(reshape(D_sub6_sub6(k,q,n,:,n_idx),[1,N_UE_sub6]))^2;
                   HI_sub6(k,n) = HI_sub6(k,n) + p_d*(1-Kr_sub6*Kt)*norm(reshape(D_sub6_sub6(k,q,n,:,n_idx),[1,N_UE_sub6]))^2;
                end
                snr_num_sub6(k,n) = DS_sub6(k,n);
                snr_den_sub6(k,n) = MSI_sub6(k,n) + MUI_sub6(k,n) + HI_sub6(k,n) + noise_sub6(k,n);
