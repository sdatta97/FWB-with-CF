...
lambda_BS = 50; %200; %densityBS
lambda_UE_sub6 = 50;%50:10:100;
rate_anal_mmW_user = zeros(size(lambda_BS));
rate_anal_mmW_user2 = zeros(size(lambda_BS));
            ...
            for idxFailureDetectionDelay = 1:length(protocolParams.FailureDetectionTime)
                for idxConnDelay = 1:length(protocolParams.connection_time)
                    for idxSignalingAfterRachDelay = 1:length(protocolParams.signalingAfterRachTime)
                        ...
                        % end
                        plos2 = zeros(params.numGNB_sub6,params.numUE);
                        % plos = ones(params.numUE,1);
                        plos = zeros(params.numUE,1);
                        ...
                        % rate_analv3 = rate_analyticalv3(params, plos2, plos,BETA,ricianFactor);
                        % rate_anal_mmW_user2(idxBSDensity) = rate_analv3(1);
