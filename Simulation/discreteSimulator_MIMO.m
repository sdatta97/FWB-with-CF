...
%                         [params.D, ue_idxs_affected] = AP_reassign(params,ue_idx);
                        ...
                        rate_dl_before_handoff = compute_link_rates_MIMO_mmse(params,channel_dl, channel_est_dl,channel_dl_mmW, channel_est_dl_mmW,ue_idx,zeros(numUE,1));                                              
...
%                         lb = quantile(rate_dl_after_handoff(ue_idxs_affected),params.lb_thres);
                        ...
                        if (all(rate_dl_after_handoff(nonzeros((1:numUE)'.*sub6ConnectionState)) >= r_min) && (lb >= r_min_sub6)) %&& (rate_dip_affected <= rate_reduce_threshold) 
