...
%                 rate_dl_after_handoff = compute_link_rates_MIMOv3(params,channel_dl, channel_est_dl,channel_dl_mmW, channel_est_dl_mmW,ue_idx,sub6ConnectionState);                                              
                ...
                result_string = ['/handoff_impact_',num2str(numUE),...
                    'UE_',num2str(lambda_BS(idxBSDensity)),...
                    'lambdaBS_',num2str(lambda_UE_sub6(idxUEDensity)),...
                    'lambdaUE_',num2str(numBlockers), 'Blockers_randomHeight_', num2str(aID),'mmse'];    
                ...
                output_categories = ['UE idx,','lambdaBS,','lambdaUE,','numBlockers,',...
                    'rate_before_handoff,','rate_after_handoff,','rate_before_handoff_affected,','rate_after_handoff_affected,','rate_mmW\n'];  
                ...
                formatSpec = '%d,%d,%d,%d,%d,%.16f,%.16f,%.16f,%.16f\n';
                fprintf(fileID,formatSpec,ue_idx, lambda_BS(idxBSDensity),lambda_UE_sub6(idxUEDensity),numBlockers,...
                   mean_rate_before_handoff,mean_rate_after_handoff,mean_rate_before_handoff_affected,mean_rate_after_handoff_affected,rate_mmW);                      
                fclose(fileID);
                % end
            end
        end
