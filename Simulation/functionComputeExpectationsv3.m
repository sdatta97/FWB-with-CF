...
for k = 1:K_mmW
    ...
    for n = 1:N_UE_mmW
        ...
        for i = 1:K_mmW
            if (i==k)
                Ck_mmW(:,:,k,i,n) = (reshape(D_mmW_mmW_wo_p(k,:,:,n),[L,N_UE_mmW]))*(reshape(Psi_mmW_2(k,:,:),[N_UE_mmW,N_UE_mmW]))'*(eyeN+reshape(D_mmW_mmW(k,k,:,:),[N_UE_mmW,N_UE_mmW])*reshape(D_mmW_mmW(k,k,:,:),[N_UE_mmW,N_UE_mmW])')*(reshape(Psi_mmW_2(k,:,:),[N_UE_mmW,N_UE_mmW]))*(reshape(D_mmW_mmW_wo_p(k,:,:,n),[L,N_UE_mmW])).';
            else
...
for k = 1:K-K_mmW
    ...
    for n = 1:N_UE_sub6
        ...
        for i = 1:K-K_mmW
            if (i==k)
                Ck_sub6(:,:,k,i+K_mmW,n) = (reshape(D_sub6_sub6_wo_p(k,:,:,n),[L,N_UE_sub6]))*(reshape(Psi_sub6_2(k,:,:),[N_UE_sub6,N_UE_sub6]))'*(eyeN+reshape(D_sub6_sub6(k,k,:,:),[N_UE_sub6,N_UE_sub6])*reshape(D_sub6_sub6(k,k,:,:),[N_UE_sub6,N_UE_sub6])')*(reshape(Psi_sub6_2(k,:,:),[N_UE_sub6,N_UE_sub6]))*(reshape(D_sub6_sub6_wo_p(k,:,:,n),[L,N_UE_sub6])).';
            else
