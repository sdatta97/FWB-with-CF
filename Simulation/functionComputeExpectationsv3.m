function [bk, Ck] = ...
    functionComputeExpectationsv3(Hhat_mmW, H_mmW, Hhat_sub6,H_sub6,D,C,nbrOfRealizations,N,N_UE_mmW,N_UE_sub6,K,K_mmW,L,L_mmW,p)
...
for n=1:nbrOfRealizations    
    %Go through all APs
    for k = 1:K_mmW
        D_mmW = zeros(N_UE_mmW,N_UE_mmW);
        D_sub6 = zeros(N_UE_mmW,N_UE_sub6);
        for i = 1:K_mmW
            if (i~=k)
                for l = 1:L
        ...
        for i = 1:K-K_mmW
            for l = 1:L
                ...
                D_sub6 = D_sub6 + H'*H_int;
            end
        end
    end
    for k = 1:K-K_mmW            
        D_mmW = zeros(N_UE_sub6,N_UE_mmW);
        D_sub6 = zeros(N_UE_sub6,N_UE_sub6);
        for i = 1:K_mmW
            for l = 1:L
