...
function rate_dl = rate_analyticalv2(params, plos2, plos, BETA, ricianFactor)
    Kd_mmw = params.numUE;
...
%         BU(k) = BU(k) + a^2*p_d*Ntx*(C_v(:,k).^2)'*((BETA(:,k)).^2);       
        DS(k) = DS(k) + a*Ntx*sqrt(p_d)*C_v(idx_ue(:,k)==1)'*BETA(idx_ue(:,k)==1,k);
        BU(k) = BU(k) + a^2*p_d*Ntx*(C_v(idx_ue(:,k)==1,k).^2)'*((BETA(idx_ue(:,k)==1,k)).^2);        
        for q = 1:Kd_mmw
             if (q~=k)
                % E(:,q) = sqrt(0.5*(b-a^2)*(C_v(:,q).^2))*(randn(M,1) + 1i*randn(M,1));
%                 MUI(k) = MUI(k) + plos(q)*a^2*Ntx*p_d*((C_v(:,q)).^2)'*(BETA(:,q).*BETA(:,k));
                % idxs = find(idx_ue(:,q).*idx_ue(:,k)==1);
                idxs = find(idx_ue(:,q)==1);
                MUI(k) = MUI(k) + plos(q)*a^2*Ntx*p_d*(C_v(idxs,q).^2)'*(BETA(idxs,q).*BETA(idxs,k));            
             end
        end
        for q = 1+Kd_mmw:Kd
            % E(:,q) = sqrt(0.5*(b-a^2)*(C_v(:,q).^2))*(randn(M,1) + 1i*randn(M,1));
...
%             MUI(k) = MUI(k) + plos(q)*a^2*p_d*Ntx*((C_v(:,q)).^2)'*(BETA(:,q).*BETA(:,k));
            % idxs = find(idx_ue(:,q).*idx_ue(:,k)==1);
            idxs = find(idx_ue(:,q)==1);
            MUI(k) = MUI(k) + plos(q)*a^2*p_d*Ntx*((C_v(idxs,q)).^2)'*(BETA(idxs,q).*BETA(idxs,k));
        end
        for q = 1+Kd_mmw:Kd
             if (q~=k)
                % E(:,q) = sqrt(0.5*(b-a^2)*(C_v(:,q).^2))*(randn(M,1) + 1i*randn(M,1));
%                 MUI(k) = MUI(k) + a^2*p_d*Ntx*((C_v(:,q)).^2)'*(BETA(:,q).*BETA(:,k));
                % idxs = find(idx_ue(:,q).*idx_ue(:,k)==1);
                idxs = find(idx_ue(:,q)==1);
                MUI(k) = MUI(k) + a^2*p_d*Ntx*((C_v(idxs,q)).^2)'*(BETA(idxs,q).*BETA(idxs,k));
             end
        end
        % TQD(k) = TQD(k) + abs(sqrt(p_d)*reshape(channel_dl(m,k,:),[Ntx,1]).'*reshape(conj(channel_est_dl(m,q,:)),[Ntx,1])*E(m,q))^2;
        % for l = 1:Ku
