...
warning('off','all');
    ...
    psi_HI_sub6 = zeros(N,N,L,K-K_mmW);   %cov matrix of pilot signal with HARDWARE IMPARIMENTS (only RX- side distortion)
    ...
    for ap = 1:L
        ...
        for ue = 1:K-K_mmW
            ...
            for ue_d = 1:K-K_mmW
                if round(PHI(:,ue+K_mmW)'*PHI(:,ue_d+K_mmW)) ==1
    ...
    y_p_mmW = zeros(N_mmW,no_of_rea,L,K_mmW);
    y_p_mean_mmW = zeros(N_mmW,no_of_rea,L,K_mmW);
    %----------
    y_p_HI_mmW = zeros(N_mmW,no_of_rea,L,K_mmW);
    h_hat_HI_mmW = zeros(N_mmW,no_of_rea,L,K_mmW);
    HI_UE_tr_mult_mmW = zeros(N_mmW,K_mmW,K_mmW);
    y_p_sub6 = zeros(N,no_of_rea,L,K-K_mmW);
    y_p_mean_sub6 = zeros(N,no_of_rea,L,K-K_mmW);
    %----------
    y_p_HI_sub6 = zeros(N,no_of_rea,L,K-K_mmW);
    h_hat_HI_sub6 = zeros(N,no_of_rea,L,K-K_mmW);
    HI_UE_tr_mult_sub6 = zeros(N,K-K_mmW,K-K_mmW);
    %--------
    ...
    for chreali = 1:no_of_rea
        for ap = 1:L
            ...
            for k = 1:K-K_mmW
                for i = 1:K-K_mmW
                    y_p_sub6(:,chreali,ap,k) = y_p_sub6(:,chreali,ap,k) + tau_p*sqrt(p)*h_sub6(:,chreali,ap,i)*(PHI(:,i+K_mmW)'*PHI(:,k+K_mmW))  ;            % pilot signal with PHI(:,k)
                    y_p_mean_sub6(:,chreali,ap,k) = y_p_mean_sub6(:,chreali,ap,k) + tau_p*sqrt(p)*h_LOS(:,ap,i)*(PHI(:,i+K_mmW)'*PHI(:,k+K_mmW));
