...
CHAN_ITER     =100;% 100;     % No of monte-carlo realizations for each user setup.
B             = 20e6;   % Bandwidth
...
%            tau_p=K;
        ...
        if ch==1
         ...
         if pft_csi==0
           [G_hat,mmse_combiner,mrc_combiner,G_HAT,H] =chan_estimate (M,K,N,G,R,alpha,p_pilot,pol,p1,M1,R_G_HAT_concat,C_G_HAT_concat,a1,Norm_factor_true);
         else
             G_hat=G;
             R_G_HAT =R;
             Q1=zeros(N,N,M);
