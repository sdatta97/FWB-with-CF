function [R] = channel_cellfree_GUE_Krician_non_zero_mean(UE,AP,N,ASD_VALUE,ASD_CORR,rayleigh,BETA,K_factor,cov_area)
...
% UE_AP_dist= cell2mat(struct2cell( load('ue_bs_dist')));
% UE_AP_dist=UE_AP_dist(1:UE,1:AP);
%---------------------------------------
...
for ue = 1:UE
    for ap = 1:AP
        ...
        if rayleigh==1  %rayleigh
            ...
            path_loss_dBm(ue,ap) = -34.53-38*log10(UE_AP_dist(ue,ap)*1000);% +shaddow_fad(ue,ap);  %Pathloss component for Rayleigh fading 30.5 36.7    34.53  38
        else
            ...
            if K_Rician(ue,ap)~=0
                probLOS(ue,ap)=((maxdistLOS-(UE_AP_dist(ue,ap)*1000))./maxdistLOS);
                shaddow_fad(ue,ap)    = sigma_sf_LOS*(sqrt(delta)*shadowing_a(ap)+ sqrt(1-delta)*shadowing_b(ue));% correlated shadow fading model from paper
                path_loss_dBm(ue,ap) = -30.18-26*log10(UE_AP_dist(ue,ap)*1000);  %Pathloss component for Rician Fading
...
elseif BETA==1
    channelGain_over_noise = 1*ones(UE,AP); % 10*rand(UE,AP); %ones(UE,AP);
