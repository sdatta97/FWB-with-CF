...
UE_SETUP      = 100;     % No of random user location setups
CHAN_ITER     =1;% 100;     % No of monte-carlo realizations for each user setup.
B             = 20e6;   % Bandwidth
...
elseif RATE==1||GEE_EPA==1||GEE_OPA==1
    ...
    P_P_DOWN_dB_PLOT = 40;
    if AP_ANT==1
        N_PLOT          = [2:2:12]/1; %No. of AP antennas
        M_PLOT          = 16;%64;
        K_PLOT          = 10;
        P_DATA_dB_PLOT  = -10;
        loop            = length(N_PLOT);
    elseif M_AP==1
        N_PLOT         =128;% 1*[128,64,32,16,8,4,2,1];
        M_PLOT         =1;%[1,2,4,8,16,32,64];%[4,8,16,32,64,128,256,512];%[16,32,64];%[16,32,64,128]; %No. of APs [1,4,8,16];%
        K_PLOT         = 5;
        P_DATA_dB_PLOT = 10;
        loop           = length(M_PLOT);
    elseif K_USER==1
    ...
    elseif TX_SNR==1
        ...
        M_PLOT         =32;
        K_PLOT         =5;
        P_DATA_dB_PLOT =[-20:10:40];%[0:10:60]; %Transmit power
    ...
    elseif PILOT_POWER==1
        ...
        K_PLOT           = 5;
        P_DATA_dB_PLOT   = -10; 
        P_P_UP_dB_PLOT   = [-30:5:20]; %Pilot power
...
%     [R_G_tmp, Gmean_tmp, R_H_tmp, Hmean_tmp, K_R, SIGMA_G, R_LT_tmp, R_LR_tmp, SIGMA_LAP_tmp, SIGMA_LU_tmp] = FD_channel_cellfree_rician(Mmax,Nmax,Kmax,ASDdeg,Rician,CORR_option,Sigma_option, noiseVariancedBm, FD);
    [channelGain_over_noise,R_1,g_LOS,K_Rician,probLOS,AP_locations,UE_locations] = channel_cellfree_GUE_Krician_non_zero_mean(max(K_PLOT),max(M_PLOT),(N_PLOT),ASDdeg,1,1,0,1,1);
...
%            tau_p=K;
           ...
           [SINR_TH_DE(:,n,ue_iters),M1,des_sig,mu_user_intf,BU] = THH_UATF(p,K,M,N ,R_G_HAT_concat,pol,C_G_HAT_concat);
        ...
        if ch==1
           for ch1=1:CHAN_ITER
            if pol==2
             ...
             [Norm_factor(:,:,ch1)] =normalization (M,K,N,R,alpha,p_pilot,pol,p1,C_G_HAT_concat);
            else
         ...
         if pft_csi==0
           [G_hat,mmse_combiner,mrc_combiner,G_HAT,H] =chan_estimate_copy (M,K,N,G,R,alpha,p_pilot,pol,p1,M1,R_G_HAT_concat,C_G_HAT_concat,a1,Norm_factor_true);
         else
             G_hat=G;
             R_G_HAT =R;
             Q1=zeros(N,N,M);
...
%                 dot_prod(ch) =(G_HAT(:,ue)'*(G4(:,ue)-G_HAT(:,ue)))/(K*N*M) +dot_prod(ch);
                      ...
                      if pol==1
                       [SIM_SE(n,ch),V_sq(1:K,n,ch),Delta_sq(1:K,n,ch),V(1:K,n,ch),Delta(1:K,n,ch)] = SIM_SE_cal(G,p,combiner,K,M,N,pol,R,alpha,R_G,G_HAT);
                      else
                       [SIM_SE(n,ch),V_sq(:,:,1:K,n,ch),Delta_sq(:,:,1:K,n,ch),V(:,:,1:K,n,ch),Delta(:,:,1:K,n,ch)] = SIM_SE_cal(G,p,combiner,K,M,N,pol,R,alpha,R_G,G_HAT);
                      end
%            end
...
%         a1=mean(a);
        for ue=1:K
            if pol==1
             SINR_UATF(n,ue) =(1-K/T_C)*log2(1 +abs(mean(V_sq(ue,n,:)))^2/(1+mean(Delta(ue,n,:))+mean(V(ue,n,:))-abs(mean(V_sq(ue,n,:)))^2));%1 +abs(mean(V_sq(ue,n,:)))^2/(1+mean(Delta(ue,n,:))+mean(V(ue,n,:))-abs(mean(V_sq(ue,n,:)))^2)
            elseif pol==2
             SINR_UATF(n,ue,ue_iters) =(1-2*K/T_C)*log2(det(eye(2) +abs(mean(V_sq(:,:,ue,n,:),5)).^2*pinv(eye(2)+mean(Delta(:,:,ue,n,:),5).*eye(2)+mean(V(:,:,ue,n,:),5).*eye(2)-abs(mean(V_sq(:,:,ue,n,:),5)).^2)) ); %eye(2) +abs(mean(V_sq(:,:,ue,n,:),5)).^2*pinv(eye(2)+mean(Delta(:,:,ue,n,:),5)+mean(V(:,:,ue,n,:),5)-abs(mean(V_sq(:,:,ue,n,:),5)).^2)
            end
        end
%         SINR_TH_UATF1(n) = sum(SINR_UATF(n,:));
% 
...
%                 plot(P_DATA_dB_PLOT(1:n),SINR_TH_UATF1(1:n),'-hb');
                hold on
                drawnow
                legend('simulated','lwr bnd')
                title(['UE SETUP =',num2str(ue_iters)])
...
%                 plot(ASD_PLOT(1:n),SINR_TH_DE(1:n),'-^g');
            ...
            elseif PILOT_POWER==1
                ...
                hold on
                plot(P_P_UP_dB_PLOT(1:n),SINR_TH_DE(1:n),'-^b');
                hold on
                drawnow
                legend('simulated','lwr bnd')
                title(['UE SETUP =',num2str(ue_iters)])               
