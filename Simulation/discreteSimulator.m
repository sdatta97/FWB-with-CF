...
function simOutputs = discreteSimulator(simInputs)
%% Inputs
...
while nextEventTime < params.simTime
    ...
    %Protocol updates
    % First do disconnection events of the second BS if any required. Since
    % if primary also gets blocked at the same time as secondary
    % it shouldn't be able to use secondary as
    % a recovery option so we disconnect secondary connection first.
    % if UE.secondaryConnectionState == 1
    %     if UE.secondaryNextEventTime == currentTime
    %         UE.secondaryConnectionState = 3;
    %         UE.secondaryConnectionStateHistory = [UE.secondaryConnectionStateHistory; UE.secondaryConnectionState];
    %         UE.secondaryNextEventTime = currentTime + UE.failureDetectionDelay;
    ...
    %         UE.secondaryEventDescriptions = [UE.secondaryEventDescriptions; {'secondaryGotBlocked-NotDetectedYet-MeasurementsGoingOn'}];
    %     end
    % elseif UE.secondaryConnectionState == 3
    ...
    %     if UE.secondaryNextEventTime == currentTime
    %         idxSecondaryBS = UE.secondaryBSIdx;
    %         isBSRecovered = link{idxSecondaryBS}.blockageStatus;
    %         if isBSRecovered
    %             UE.secondaryConnectionState = 1;
    %             UE.secondaryConnectionStateHistory = [UE.secondaryConnectionStateHistory; UE.secondaryConnectionState];
    %             UE.secondaryConnectionStarts = [UE.secondaryConnectionStarts, currentTime];
    %             UE.secondaryBSHistory = [UE.secondaryBSHistory, idxSecondaryBS];
    %             UE.secondaryBSIdx = idxSecondaryBS;
    %             next_blockage = link{idxSecondaryBS}.discoveredTimes(3,find(link{idxSecondaryBS}.discoveredTimes(3,:)> currentTime,1));
    %             UE.secondaryNextEventTime = next_blockage;
    %             UE.secondaryEventTimes = [UE.secondaryEventTimes; currentTime];
    %             UE.secondaryEventDescriptions =  [UE.secondaryEventDescriptions; {'MeasurementDone-Recovered-b4-BFDT-expired-No-Failure-ReconnectedWithoutDelays'}];
    %         else
    ...
    %             % re-establishment. 
    %             UE.secondaryConnectionState = 4;
    %             UE.secondaryConnectionStateHistory = [UE.secondaryConnectionStateHistory; UE.secondaryConnectionState];
    %             UE.secondaryNextEventTime = currentTime + UE.beamFailureRecoveryTimer;
    %             % In order to give this BS a priority in the future
    %                 lastConnectedBS = UE.secondaryBSHistory(end);
    %                 bsLastConnectionTimes(lastConnectedBS) = currentTime;
    %                 [~,bsPriorities] = sort(bsLastConnectionTimes,'descend');
    %             % now event recording
    %             UE.secondaryEventTimes = [UE.secondaryEventTimes; currentTime];
    %             UE.secondaryEventDescriptions =  [UE.secondaryEventDescriptions; {'MeasurementDone-Declared-Beam-Failure-going2-BeamRecovery'}];
    %         end
    %     end
    % elseif UE.secondaryConnectionState == 4
    %     if UE.secondaryNextEventTime == currentTime
    %         %Check if BS recovered
    %         idxSecondaryBS = UE.secondaryBSIdx;
    %         isBSRecovered = link{idxSecondaryBS}.blockageStatus;
    %         if isBSRecovered
    %             if link{idxSecondaryBS}.blockageStatus %&& link{idxSecondaryBS}.discovery_state
    %                 UE.secondaryConnectionState = 2;
    %                 UE.secondaryConnectionStateHistory = [UE.secondaryConnectionStateHistory; UE.secondaryConnectionState];
    %                 UE.secondaryTargetIdx = idxSecondaryBS;
    %                 UE.secondaryBSIdx= [];
    %                 UE.secondaryNextEventTime = currentTime + UE.RACH_eff;
    %                 link{idxSecondaryBS}.nextEventTime = currentTime + UE.RACH_eff;
    %                 % now event recording
    %                 UE.secondaryEventTimes = [UE.secondaryEventTimes; currentTime];
    %                 UE.secondaryEventDescriptions =  [UE.secondaryEventDescriptions; {'Recovered-b4-RLF-expired-(Re)Connecting-to-BS'}];
    %             else
    %                 disp('Arrived this line something might be wrong. DEBUG')
    %             end
    %         else
    %             %Delcare RLF to go idle state
    %             UE.secondaryConnectionState = 0;
    %             UE.secondaryConnectionStateHistory = [UE.secondaryConnectionStateHistory; UE.secondaryConnectionState];
    %             UE.secondaryBSIdx = [];
    %             % now event recording
    %             UE.secondaryEventTimes = [UE.secondaryEventTimes; currentTime];
    %             UE.secondaryEventDescriptions =  [UE.secondaryEventDescriptions; {'Declared-RLF-Going2-NonConnectedState-CanConnect-any-available-BS'}];
    %         end
    %     end
    % end
    for ue_idx = 1:numUE
...
%         if UE.primaryNextEventTime == currentTime
%             UE.primaryConnectionState = 3;
%             UE.primaryConnectionStateHistory = [UE.primaryConnectionStateHistory; UE.primaryConnectionState];
%             UE.primaryNextEventTime = currentTime + UE.failureDetectionDelay;
%             UE.primaryConnectionEnds = [UE.primaryConnectionEnds, currentTime];
%             % [~, phy_channel_sub6] = computePhysicalChannels(params);
%             % phy_channel_sub6 = computePhysicalChannels_sub6(params);
%             % link_rates_sub6 = compute_link_rates(phy_channel_sub6, scs_sub6,0, ue_idx);
%             [BETA, phy_channel_sub6, phy_channel_sub6_est] = computePhysicalChannels_sub6v2(params);
%             ue_rates = compute_link_ratesv2(BETA, phy_channel_sub6, phy_channel_sub6_est, params.scs_sub6);
%             if (ue_rates(ue_idx) > r_min)
%                 UE.sub6ConnectionStarts = [UE.sub6ConnectionStarts, currentTime];
%             end
%             UE.primaryEventTimes = [UE.primaryEventTimes; currentTime];
%             UE.primaryEventDescriptions = [UE.primaryEventDescriptions; {'primaryGotBlocked-NotDetectedYet-MeasurementsGoingOn-DataplaneInterruption'}];
%         end
%     elseif UE.primaryConnectionState == 3
%         if UE.primaryNextEventTime == currentTime
%             idxPrimaryBS = UE.primaryBSIdx;
%             isBSRecovered = link{idxPrimaryBS}.blockageStatus;
%             if isBSRecovered
%                 UE.primaryConnectionState = 1;
%                 UE.primaryConnectionStateHistory = [UE.primaryConnectionStateHistory; UE.primaryConnectionState];
...
%                 end
%                 UE.primaryBSHistory = [UE.primaryBSHistory, idxPrimaryBS];
%                 UE.primaryBSIdx = idxPrimaryBS;
%                 next_blockage = link{idxPrimaryBS}.discoveredTimes(3,find(link{idxPrimaryBS}.discoveredTimes(3,:)> currentTime,1));
%                 UE.primaryNextEventTime = next_blockage;
%                 UE.primaryEventTimes = [UE.primaryEventTimes; currentTime];
%                 UE.primaryEventDescriptions =  [UE.primaryEventDescriptions; {'MeasurementDone-Recovered-b4-BFDT-expired-No-Failure-ReconnectedWithoutDelays'}];
%             else
%                 %BS didnt recover now we declare beam faiure and go to
...
%                     % In order to give this BS a priority in the future
%                         lastConnectedBS = UE.primaryBSHistory(end);
%                         bsLastConnectionTimes(lastConnectedBS) = currentTime;
%                         [~,bsPriorities] = sort(bsLastConnectionTimes,'descend');
%                     % now event recording
%                     UE.primaryEventTimes = [UE.primaryEventTimes; currentTime];
%                     UE.primaryEventDescriptions =  [UE.primaryEventDescriptions; {'MeasurementDone,SecondaryBS-Available-Initate-MCG-Fail-Recovery'}];
%                 elseif UE.secondaryConnectionState == 2
...
%                     UE.primaryEventTimes = [UE.primaryEventTimes; currentTime];
%                     UE.primaryEventDescriptions = [UE.primaryEventDescriptions; {'MeasurementsDone,SecondaryRACH-Aborted,now-the-sameBS used as primary RACH.'}];
%                 else
%                     UE.primaryConnectionState = 4;
%                     UE.primaryConnectionStateHistory = [UE.primaryConnectionStateHistory; UE.primaryConnectionState];
%                     UE.primaryNextEventTime = currentTime + UE.beamFailureRecoveryTimer;
%                     % In order to give this BS a priority in the future
%                         lastConnectedBS = UE.primaryBSHistory(end);
%                         bsLastConnectionTimes(lastConnectedBS) = currentTime;
%                         [~,bsPriorities] = sort(bsLastConnectionTimes,'descend');
%                     % now event recording
%                     UE.primaryEventTimes = [UE.primaryEventTimes; currentTime];
%                     UE.primaryEventDescriptions =  [UE.primaryEventDescriptions; {'MeasurementDone,-No-Secondary-BS-Available-4-MCG-Failure-Recovery,-Declared-Beam-Failure-going2-BeamRecovery'}];
%                 end
%             end            
%         end
%     elseif UE.primaryConnectionState == 4
%         if UE.primaryNextEventTime == currentTime
%             idxPrimaryBS = UE.primaryBSIdx;
...
%             end
%             if isBSRecovered
%                 if link{idxPrimaryBS}.blockageStatus %&& link{idxPrimaryBS}.discovery_state
%                     UE.primaryConnectionState = 2;
%                     UE.primaryConnectionStateHistory = [UE.primaryConnectionStateHistory; UE.primaryConnectionState];
%                     UE.primaryTargetIdx = idxPrimaryBS;
%                     UE.primaryNextEventTime = currentTime + UE.RACH_eff;
%                     link{idxPrimaryBS}.nextEventTime = currentTime + UE.RACH_eff;
%                     % now event recording
%                     UE.primaryEventTimes = [UE.primaryEventTimes; currentTime];
%                     UE.primaryEventDescriptions =  [UE.primaryEventDescriptions; {'Recovered-b4-RLF-expired-(Re)Connecting-to-BS'}];
%                 else
%                     disp('Arrived this line something might be wrong. DEBUG')
%                 end
%             else
...
%             end
%         end
%     elseif UE.primaryConnectionState == 5
%         if UE.primaryNextEventTime == currentTime
% %             secondaryBSIdx = UE.secondaryBSIdx;
%             secondaryBSIdx = UE.tmpMCGBSIdx;
%             if link{UE.tmpMCGBSIdx}.blockageStatus && link{UE.tmpMCGBSIdx}.discovery_state
...
%                 UE.secondaryConnectionEnds = [UE.secondaryConnectionEnds, currentTime];
%                 UE.secondaryEventTimes = [UE.secondaryEventTimes; currentTime];
%                 UE.secondaryEventDescriptions = [UE.secondaryEventDescriptions; {'primaryBlocked-and-Secondary-Changed-to-Primary'}];
%                 % Add to primary
%                 UE.primaryConnectionState =1;
%                 UE.primaryConnectionStateHistory = [UE.primaryConnectionStateHistory; UE.primaryConnectionState];
%                 UE.primaryConnectionStarts = [UE.primaryConnectionStarts, currentTime];
...
%                     UE.sub6ConnectionEnds = [UE.sub6ConnectionEnds, currentTime];
%                 end
%                 UE.primaryBSIdx = secondaryBSIdx;
%                 UE.primaryBSHistory = [UE.primaryBSHistory, UE.primaryBSIdx];
%                 next_blockage = link{secondaryBSIdx}.discoveredTimes(3,find(link{secondaryBSIdx}.discoveredTimes(3,:)> currentTime,1));
%                 UE.primaryNextEventTime = next_blockage;
%                 UE.primaryEventTimes = [UE.primaryEventTimes; currentTime];
%                 UE.primaryEventDescriptions =  [UE.primaryEventDescriptions; {'primaryGotBlocked-Secondary-Become-Primary-BS-Completed'}];
%             else
...
%                 % now event recording
%                 UE.primaryEventTimes = [UE.primaryEventTimes; currentTime];
%                 UE.primaryEventDescriptions =  [UE.primaryEventDescriptions; {'Secondary BS also have problems, couldnt transfer secondary to primary. Trying BFR with primary.'}];
%             end
%         end
%     end
    for ue_idx = 1:numUE
            ...
            if UE.primaryNextEventTime(ue_idx) == currentTime
                ...
                if isBSRecovered
                    ...
                    if UE.sub6ConnectionState(ue_idx) == 1
                        ...
                        r_calc_sub6 = compute_link_ratesv3(BETA, phy_channel_sub6, phy_channel_sub6_est, params.scs_sub6,ue_idx,UE.sub6ConnectionState);
    ...
    % if UE.primaryConnectionState == 0
    %     UE = tryPrimaryConnecting(UE,currentTime,link,bsPriorities,bsLastConnectionTimes);
    % elseif UE.primaryConnectionState == 2
    %     if UE.primaryNextEventTime ~= currentTime
    % 
    %     else
    %         idxBS = UE.primaryTargetIdx;
    ...
    %                 UE.sub6ConnectionEnds = [UE.sub6ConnectionEnds, currentTime];
    %             end
    %             UE.primaryBSHistory = [UE.primaryBSHistory, idxBS];
    %             UE.primaryBSIdx = idxBS;
    %             next_blockage = link{idxBS}.discoveredTimes(3,find(link{idxBS}.discoveredTimes(3,:)> currentTime,1));
    %             UE.primaryNextEventTime = next_blockage;
    %             UE.primaryEventTimes = [UE.primaryEventTimes; currentTime];
    %             UE.primaryEventDescriptions =  [UE.primaryEventDescriptions; {'primaryRachCompleted-Connected'}];
    %         else
    %             UE.primaryConnectionState = 0;
    %             UE.primaryConnectionStateHistory = [UE.primaryConnectionStateHistory; UE.primaryConnectionState];
    %             UE.primaryEventTimes = [UE.primaryEventTimes; currentTime];
    %             UE.primaryEventDescriptions =  [UE.primaryEventDescriptions; {'primaryRachFailed-NotConnected-go2-Idle-State-Can-Try-another-BS'}];
    %             % UE.sub6ConnectionStarts = [UE.sub6ConnectionStarts, currentTime];
    %             UE = tryPrimaryConnecting(UE,currentTime,link,bsPriorities,bsLastConnectionTimes);
    %         end
    %     end
    ...
    % if UE.secondaryConnectionState == 0
    %     UE = trySecondaryConnecting(UE,currentTime,link,bsPriorities,bsLastConnectionTimes);
    % elseif UE.secondaryConnectionState == 2
    %     if UE.secondaryNextEventTime ~= currentTime
    % 
    %     else
    %         idxBS = UE.secondaryTargetIdx;
    %         if link{idxBS}.discovery_state
    %             UE.secondaryConnectionState = 1;
    %             UE.secondaryConnectionStateHistory = [UE.secondaryConnectionStateHistory; UE.secondaryConnectionState];
    %             UE.secondaryTargetIdx = [];
    %             UE.secondaryConnectionStarts = [UE.secondaryConnectionStarts, currentTime];
    %             UE.secondaryBSHistory = [UE.secondaryBSHistory, idxBS];
    %             UE.secondaryBSIdx = idxBS;
    %             next_blockage = link{idxBS}.discoveredTimes(3,find(link{idxBS}.discoveredTimes(3,:)> currentTime,1));
    %             UE.secondaryNextEventTime = next_blockage;
    %             UE.secondaryEventTimes = [UE.secondaryEventTimes; currentTime];
    %             UE.secondaryEventDescriptions =  [UE.secondaryEventDescriptions; {'secondaryRachCompleted-Connected'}];
    %         else
    %             UE.secondaryConnectionState = 0;
    %             UE.secondaryConnectionStateHistory = [UE.secondaryConnectionStateHistory; UE.secondaryConnectionState];
    %             UE.secondaryEventTimes = [UE.secondaryEventTimes; currentTime];
    %             UE.secondaryEventDescriptions =  [UE.secondaryEventDescriptions; {'secondaryRachFailed-NotConnected-go2-Idle-State-Can-Try-another-BS'}];
    %             UE = trySecondaryConnecting(UE,currentTime,link,bsPriorities,bsLastConnectionTimes);
    %         end
    %     end
    % end
    ...
    nextEventTimes = getNextEvents(link,params.simTime);
    % nextEventTime = min(nextEventTimes(:));
    % nextEventTime = min(nextEventTimes(1:numUE,:),[],2);
    nextEventTime = params.simTime;
    for ue_idx = 1:numUE
        % if (UE.primaryBSIdx(ue_idx) > 0)
        %     nextEventTime = min(nextEventTime,nextEventTimes(ue_idx,UE.primaryBSIdx(ue_idx)));
        % elseif (UE.primaryTargetIdx(ue_idx) > 0)
        %     nextEventTime = min(nextEventTime,nextEventTimes(ue_idx,UE.primaryTargetIdx(ue_idx)));
        % end
        % elseif (UE.secondaryBSIdx(ue_idx) > 0)
        %     nextEventTime = min(nextEventTime,nextEventTimes(ue_idx,UE.secondaryBSIdx(ue_idx)));
        % end
        % elseif (UE.secondaryTargetIdx(ue_idx) > 0)
        %     nextEventTime = min(nextEventTime,nextEventTimes(ue_idx,UE.secondaryTargetIdx(ue_idx)));
        % end
        if (min(nextEventTimes(ue_idx,1:params.numGNB)) < nextEventTime)
            nextEventTime = min(nextEventTimes(ue_idx,1:params.numGNB));
        % else
        %     nextEventTime = min(nextEventTime,min(nextEventTimes(ue_idx,1:params.numGNB)));
        end
    end
    %need to check Ue event times as well
    UEEventTimes = [UE.primaryNextEventTime;UE.secondaryNextEventTime];
...
for ue_idx = 1:numUE
    ...
    % UE.primaryConnectionEnds = [UE.primaryConnectionEnds, params.simTime*ones(1,numUE)];
    % UE.primaryConnectionEndIndices = [UE.primaryConnectionEndIndices, UE.primaryConnectionStartIndices(1+length(UE.primaryConnectionEndIndices):end)];
        % UE.primaryConnectionEnds = [UE.primaryConnectionEnds, params.simTime*ones(1,length(UE.primaryConnectionStarts(UE.primaryConnectionStartIndices==ue_idx))-length(UE.primaryConnectionEnds(UE.primaryConnectionEndIndices==ue_idx)))];
        UE.primaryConnectionEnds = [UE.primaryConnectionEnds, params.simTime];
    % for ue_idx = 1:numUE
        % if(sum(UE.primaryConnectionStartIndices==ue_idx) > sum(UE.primaryConnectionEndIndices==ue_idx))
        UE.primaryConnectionEndIndices = [UE.primaryConnectionEndIndices, ue_idx];
        % end
    end
end
...
for ue_idx = 1:numUE
    ...
    % UE.secondaryConnectionEnds = [UE.secondaryConnectionEnds, params.simTime*ones(1,numUE)];
    % UE.secondaryConnectionEndIndices = [UE.secondaryConnectionEndIndices, UE.secondaryConnectionStartIndices(1+length(UE.secondaryConnectionEndIndices):end)];
        % UE.secondaryConnectionEnds = [UE.secondaryConnectionEnds, params.simTime*ones(1,length(UE.secondaryConnectionStarts(UE.secondaryConnectionStartIndices==ue_idx))-length(UE.secondaryConnectionEnds(UE.secondaryConnectionEndIndices==ue_idx)))];
        UE.secondaryConnectionEnds = [UE.secondaryConnectionEnds, params.simTime];
    % for ue_idx = 1:numUE
        % if(sum(UE.secondaryConnectionStartIndices==ue_idx) > sum(UE.secondaryConnectionEndIndices==ue_idx))
        UE.secondaryConnectionEndIndices = [UE.secondaryConnectionEndIndices, ue_idx];
        % end
    end
end
