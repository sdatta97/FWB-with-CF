...
for k = 1:K
        ...
        for ch = 1:no_of_rea    
            ...
            for ap =1:L %a-th AP
                ...
                for kk = 1:K
                    ...
                    HI_AP_variance = HI_AP_variance + K_AP_TR*eta(ap,kk)*diag(diag(h_hat(:,ch,ap,kk)*h_hat(:,ch,ap,kk)')); % sum over UEs -- a-th AP HW 
    ...
    %                 HI_AP_tx_monte(k,ch,ap,kk) =K_AP_TR*eta(ap,kk)*(h(:,ch,ap,k)'*diag(diag(h_hat(:,ch,ap,kk)*h_hat(:,ch,ap,kk)'))*h(:,ch,ap,k)); %closed-frominsome
                ...
                warning('off','all');
                HI_AP_tx_monte(k,ch) = HI_AP_tx_monte(k,ch) + (h(:,ch,ap,k)'* sqrtm(HI_AP_variance)*sqrt(0.5)*(randn(N,1)+1i*randn(N,1)) ); %sum over APs
            end
