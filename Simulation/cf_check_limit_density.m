...
for aID = 1:9
    ...
    for idxdeployRange = 1:length(params.deployRange)
        ...
        % numUE_sub6_arr = 10;
        lambda_UE_sub6 = 1500:500:2500; %1000:1000:10000; % [30:20:90, 100]; %:100:2000;
                    ...
                    for idx_p = 1:length(p_fac_arr)
                        for idxlbthres = 1:length(lb_thresh)
                            ...
                            result_string = strcat('/handoff_impact_',num2str(numUE),...
                                ...
                                'deployRange_','randomHeight_', num2str(aID),'Min_rate', num2str(rmin), "Pow_fac", num2str(params.p_fac), "lb_thres", num2str(100*params.lb_thres));
                            ...
                            output_categories = ['lambdaBS,','lambdaUE,','numBlockers,',...
                                'deployRange,','minRatereq,','powerFac,','lower_bound_thresh,', 'lb_rate_before_handoff,','bw_allotted\n'];
                    
                            fprintf(fileID,output_categories);
                        
                            p_fac = params.p_fac;
                            formatSpec = '%d,%d,%d,%f,%f,%f,%f,%.16f,%.16f\n';
                            fprintf(fileID,formatSpec,lambda_BS(idxBSDensity),lambda_UE_sub6(idxUEDensity),numBlockers,...
                                deployRange,rmin, p_fac, lb_thres,lb,bw_alloc);
                            fclose(fileID);
                        end
                    end
                end
