...
w_list = 1000/50;      %Connection establishment times
dt_list = 1000/50;   %Discovery Times
self_blockage = 5/6;


P_OS = zeros(length(lambda_BS),length(K_list),length(w_list),length(dt_list),length(lambda_B));
...
for indBS=1:length(lambda_BS)
    ...
    for M = 1:M_max
        ...
        RgNB = sort(R*sqrt(rand(M,1))); %location of gNBs (distance from origin)
        for indK = 1:length(K_list)
            ...
            for indW = 1:length(w_list)
                ...
                for indDt = 1:length(dt_list)
                    dt = dt_list(indDt);
                    u = 1/(1/mu + 1/dt);
                    for indA = 1:length(lambda_B)
                        index = 0;
                        for idxM = 0:M
                            % a = bl_rate_calc(C(indA),R,0,M);    %blocker arrical rates
                            a = bl_rate_calc(C(indA),RgNB,idxM,M,mu);    %blocker arrical rates
                            K_lim = min(K,idxM);
                            for idxK = 0:K_lim
                                index = index+1;
                                disp([idxM,idxK])
...
save(string,'description','P_OS','lambda_BS','K_list','w_list','dt_list','lambda_B');
